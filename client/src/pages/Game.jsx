// ============================================================
// pages/Game.jsx — Schermata di gioco Tic Tac Toe
// ============================================================
...
export default function Game({ initialGameState, player, onLeave }) {
  ...
  useEffect(() => {
    ...
    const handleGameAborted = ({ message: msg }) => {
      setMessage(msg);
    };

    const handleRematchReady = ({ room }) => {
      onLeave("lobby", room);
    };
  ...
  function updateMessage(gs) {
    if (gs.status === "finished") {
      if (gs.winner === "draw") {
        setMessage("Pareggio! Nessuno vince.");
      } else if (gs.winner === socket.id) {
        setMessage("🎉 Hai vinto!");
      } else {
        setMessage("Hai perso. Meglio la prossima volta!");
      }
      return;
    }
    ...
    <div className="game-page">
      ...
      <div className="scoreboard">
        {gameState.players.map((p) => (
          <div
            key={p.id}
            className={`score-player ${gameState.currentTurn === p.id && !isFinished ? "active-turn" : ""}`}
          >
      ...
      {isFinished && (
        <div className="game-over-actions">
          ...
          <button className="btn btn-ghost" onClick={() => onLeave("home")}>
            ⌂ Home
          </button>
        </div>
      )}
